	...
	Factory.Project = f.NewTable("sqlkite_projects", func(args f.KV) f.KV {
		...
		return f.KV{
			...
			"data": f.JSON{
				...
				"limits": f.JSON{
					"max_concurrency":         args.Int("limits.max_concurrency", 2),
					"max_sql_length":          args.Int("limits.max_sql_length", 4000),
					"max_sql_parameter_count": args.Int("limits.max_sql_parameter_count", 100),
					"max_database_size":       args.Int("limits.max_database_size", 100000),
					"max_select_count":        args.Int("limits.max_select_count", 100),
					"max_result_length":       args.Int("limits.max_result_length", 10000),
					"max_from_count":          args.Int("limits.max_from_count", 10),
					"max_select_column_count": args.Int("limits.max_select_column_count", 10),
					"max_condition_count":     args.Int("limits.max_condition_count", 10),
					"max_order_by_count":      args.Int("limits.max_order_by_count", 5),
					"max_table_count":         args.Int("limits.max_table_count", 10),
				},
				"auth": f.JSON{
					"disabled": args.Bool("auth.disabled", false),
					"session": f.JSON{
						"session_ttl": args.Int("auth.session.session_ttl", 0),
					},
				},
			},
		}
